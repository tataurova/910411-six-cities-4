...
const Sort = (props) => {
  ...
  return (
    <form className="places__sorting" action="#" method="get">
      ...
      <span className="places__sorting-type" tabIndex="0" onClick={() => props.onChangeItem(!props.state)}>
        {getSortTypeName(activeSortType)}
        <svg className="places__sorting-arrow" width="7" height="4">
          <use xlinkHref="#icon-arrow-select"></use>
        </svg>
      </span>
      <ul className={`places__options places__options--custom ${props.state ? `places__options--opened` : ``}`}>
        {SORT_TYPES.map((sortType, index) => (
          <li
            ...
            onClick={(evt) => {
              props.onSortTypeClick(evt.target.dataset.sort);
              props.onChangeItem(!props.state);
            }}>
            {sortType}
          </li>
        ))}
